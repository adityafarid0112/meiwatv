...
<body>
  ...
  function setHeroMatchPreview(match) {
    ...
    // Update URL parameter di browser agar link pertandingan sinkron
    try {
      const slug = (match.title || match.id || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
      const newUrl = window.location.pathname + '?match=' + encodeURIComponent(slug);
  ...
  function initLivePortal() {
    let initialMatchParam = null;
    try {
      const urlParams = new URLSearchParams(window.location.search);
      initialMatchParam = urlParams.get('match') || urlParams.get('m_id');
      if (!initialMatchParam && window.location.hash) {
        initialMatchParam = window.location.hash.replace('#', '').replace('match=', '').replace('match-', '');
      }
    } catch (_) {}

    let targetMatch = null;
    let targetIdx = -1;
    if (initialMatchParam && AUTO_LIVE_DATASET.length > 0) {
      targetIdx = findMatchByKeywordOrSlug(initialMatchParam);
      if (targetIdx >= 0) {
        targetMatch = AUTO_LIVE_DATASET[targetIdx];
      }
    }
    ...
    if (targetMatch) {
      ...
      currentActiveMatch = targetMatch;
    } else {
      ...
      if (AUTO_LIVE_DATASET.length > 0) {
        currentActiveMatch = AUTO_LIVE_DATASET[0];
      }
    }
    ...
    if (currentActiveMatch) {
      setHeroMatchPreview(currentActiveMatch);
      if (targetMatch && targetMatch.status === 1) {
        playMatchDirect(targetIdx, 1);
      }
    }
