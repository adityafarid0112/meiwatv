...
<body>
  ...
  function triggerAdsterra() {
    try {
      if (typeof window.openAdsterraPopup === 'function') {
  ...
  function renderInteractivePlaylist(matchesToRender) {
    ...
    // Auto-load first match into preview if not set
  ...
  function openCurrentMatchNewTab() {
    ...
    triggerAdsterra();
    if (currentActiveMatch) {
      const url = currentActiveMatch.postUrl || currentActiveMatch.streamJalur1 || window.location.href;
      window.open(url, '_blank', 'noopener,noreferrer');
    }
  }

  function shareCurrentMatch() {
    if (navigator.share && currentActiveMatch) {
      navigator.share({
  ...
  // 7. REAL-TIME MATCHES SYNC FROM GITHUB
  ...
  function initLivePortal() {
    ...
    // Auto-select match pertama & siapkan player
